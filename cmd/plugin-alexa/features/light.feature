Feature: Light Entity
  ...
  Scenario: ToAlexa endpoint has correct capabilities
    Given a light entity "test.dev1.toalexa" named "Alexa Light" with power on brightness 200 temperature 4000
    When I translate "test.dev1.toalexa" to Alexa endpoint
    Then the Alexa display category is "LIGHT"
    And the Alexa endpoint has capability "Alexa.PowerController"
    And the Alexa endpoint has capability "Alexa.BrightnessController"
    And the Alexa endpoint has capability "Alexa.ColorTemperatureController"

  Scenario: ToAlexa state has correct properties
    Given a light entity "test.dev1.tostate" named "Alexa Light" with power on brightness 127 temperature 0
    When I translate "test.dev1.tostate" to Alexa state
    Then the Alexa property "Alexa.PowerController"/"powerState" is "ON"

  Scenario: FromAlexa TurnOn maps to domain command
    When Alexa sends "Alexa.PowerController" "TurnOn" to entity type "light"
    Then the Alexa domain command type is "LightTurnOn"

  Scenario: FromAlexa SetBrightness maps to domain command
    When Alexa sends "Alexa.BrightnessController" "SetBrightness" to entity type "light" with payload '{"brightness": 50}'
    Then the Alexa domain command type is "LightSetBrightness"
    And the Alexa domain command field "Brightness" equals 127

  Scenario: FromAlexa SetColor maps to domain command
    When Alexa sends "Alexa.ColorController" "SetColor" to entity type "light" with payload '{"color": {"hue": 120, "saturation": 0.5, "brightness": 1.0}}'
    Then the Alexa domain command type is "LightSetHS"
    And the Alexa domain command field "Hue" equals 120
