Feature: Switch Entity
  ...
  Scenario: FromAlexa TurnOn maps to SwitchTurnOn
    When Alexa sends "Alexa.PowerController" "TurnOn" to entity type "switch"
    Then the Alexa domain command type is "SwitchTurnOn"

  Scenario: FromAlexa TurnOff maps to SwitchTurnOff
    When Alexa sends "Alexa.PowerController" "TurnOff" to entity type "switch"
    Then the Alexa domain command type is "SwitchTurnOff"

  Scenario: ToAlexa endpoint has PowerController
    Given a switch entity "test.dev1.toalexa" named "Alexa Switch" with power on
    When I translate "test.dev1.toalexa" to Alexa endpoint
    Then the Alexa display category is "SWITCH"
