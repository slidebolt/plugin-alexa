...
type bddCtx struct {
	...
	// Alexa-specific
	lastAlexaEndpoint translate.AlexaEndpoint
	lastAlexaProps    []translate.AlexaProperty
	lastFromAlexaCmd  any
	lastFromAlexaErr  error
}
...
func (c *bddCtx) aClimateEntity(key, name, hvacMode string, temperature float64) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
...
func (c *bddCtx) aClimateEntityFull(key, name, hvacMode string, temperature float64, unit string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
...
func (c *bddCtx) updateClimate(key, hvacMode string, temperature float64) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
...
func (c *bddCtx) climateTemperatureIs(expected float64) error {
	if c.lastGetErr != nil {
		return fmt.Errorf("retrieve failed: %w", c.lastGetErr)
	}
	st, ok := c.lastEntity.State.(domain.Climate)
...
// ---------------------------------------------------------------------------
// Alexa translation steps
// ---------------------------------------------------------------------------

// ToAlexa: translate entity to Alexa endpoint
func (c *bddCtx) iTranslateToAlexaEndpoint(key string) error {
	e, err := c.getEntity(key)
	if err != nil {
		return err
	}
	c.lastAlexaEndpoint = translate.ToAlexa(e)
	return nil
}

func (c *bddCtx) theAlexaEndpointIDIs(expected string) error {
	if c.lastAlexaEndpoint.EndpointID != expected {
		return fmt.Errorf("endpointId: got %q, want %q", c.lastAlexaEndpoint.EndpointID, expected)
	}
	return nil
}

func (c *bddCtx) theAlexaDisplayCategoryIs(expected string) error {
	if len(c.lastAlexaEndpoint.DisplayCategories) == 0 {
		return fmt.Errorf("no display categories")
	}
	if c.lastAlexaEndpoint.DisplayCategories[0] != expected {
		return fmt.Errorf("display category: got %q, want %q", c.lastAlexaEndpoint.DisplayCategories[0], expected)
	}
	return nil
}

func (c *bddCtx) theAlexaEndpointHasCapability(iface string) error {
	for _, cap := range c.lastAlexaEndpoint.Capabilities {
		if cap.Interface == iface {
			return nil
		}
	}
	return fmt.Errorf("missing capability %q", iface)
}

// AlexaState: translate entity to Alexa state properties
func (c *bddCtx) iTranslateToAlexaState(key string) error {
	e, err := c.getEntity(key)
	if err != nil {
		return err
	}
	c.lastAlexaProps = translate.AlexaState(e)
	return nil
}

func (c *bddCtx) theAlexaPropertyIs(namespace, name, expected string) error {
	for _, p := range c.lastAlexaProps {
		if p.Namespace == namespace && p.Name == name {
			got := fmt.Sprintf("%v", p.Value)
			if got != expected {
				return fmt.Errorf("%s/%s: got %v, want %s", namespace, name, p.Value, expected)
			}
			return nil
		}
	}
	return fmt.Errorf("missing property %s/%s", namespace, name)
}

// FromAlexa: translate Alexa directive to domain command
func (c *bddCtx) alexaSendsDirective(entityType, namespace, name string) error {
	c.lastFromAlexaCmd, c.lastFromAlexaErr = translate.FromAlexa(entityType, namespace, name, nil)
	return nil
}

func (c *bddCtx) alexaSendsDirectiveWithPayload(entityType, namespace, name, payloadJSON string) error {
	var payload map[string]any
	if err := json.Unmarshal([]byte(payloadJSON), &payload); err != nil {
		return fmt.Errorf("parse payload: %w", err)
	}
	c.lastFromAlexaCmd, c.lastFromAlexaErr = translate.FromAlexa(entityType, namespace, name, payload)
	return nil
}

func (c *bddCtx) theDomainCommandTypeIs(expected string) error {
	if c.lastFromAlexaErr != nil {
		return fmt.Errorf("fromAlexa failed: %w", c.lastFromAlexaErr)
	}
	if c.lastFromAlexaCmd == nil {
		return fmt.Errorf("no command returned")
	}
	typ := reflect.TypeOf(c.lastFromAlexaCmd).Name()
	if typ != expected {
		return fmt.Errorf("command type: got %q, want %q", typ, expected)
	}
	return nil
}

func (c *bddCtx) theAlexaDomainCommandFieldEquals(field string, expected float64) error {
	if c.lastFromAlexaCmd == nil {
		return fmt.Errorf("no command")
	}
	v := reflect.ValueOf(c.lastFromAlexaCmd)
	...
	case reflect.Float64:
		got = f.Float()
	default:
		return fmt.Errorf("field %q is %s, not numeric", field, f.Kind())
	}
	if got != expected {
		return fmt.Errorf("field %q: got %v, want %v", field, got, expected)
	}
	return nil
}
...
func (c *bddCtx) RegisterSteps(ctx *godog.ScenarioContext) {
	...
	// --- Alexa: ToAlexa / FromAlexa ---
	ctx.Step(`^I translate "([^"]*)" to Alexa endpoint$`, c.iTranslateToAlexaEndpoint)
	ctx.Step(`^the Alexa endpoint ID is "([^"]*)"$`, c.theAlexaEndpointIDIs)
	ctx.Step(`^the Alexa display category is "([^"]*)"$`, c.theAlexaDisplayCategoryIs)
	ctx.Step(`^the Alexa endpoint has capability "([^"]*)"$`, c.theAlexaEndpointHasCapability)
	ctx.Step(`^I translate "([^"]*)" to Alexa state$`, c.iTranslateToAlexaState)
	ctx.Step(`^the Alexa property "([^"]*)"/"([^"]*)" is "([^"]*)"$`, c.theAlexaPropertyIs)
	ctx.Step(`^Alexa sends "([^"]*)" "([^"]*)" to entity type "([^"]*)"$`, func(namespace, name, entityType string) error {
		return c.alexaSendsDirective(entityType, namespace, name)
	})
	ctx.Step(`^Alexa sends "([^"]*)" "([^"]*)" to entity type "([^"]*)" with payload '([^']*)'$`, func(namespace, name, entityType, payload string) error {
		return c.alexaSendsDirectiveWithPayload(entityType, namespace, name, payload)
	})
	ctx.Step(`^the Alexa domain command type is "([^"]*)"$`, c.theDomainCommandTypeIs)
	ctx.Step(`^the Alexa domain command field "([^"]*)" equals ([\d.]+)$`, c.theAlexaDomainCommandFieldEquals)
}
